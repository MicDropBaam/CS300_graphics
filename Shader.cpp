	...
	m_infoLogLength(0), m_result(GL_FALSE), isGeoUse(GeoUse), m_norm(type)
{
	...
	if (type == PHONG_LIGHTING)
	{
		CreateShader("shaders\\phong_lighting_vert.vert", "shaders\\phong_lighting_frag.frag", Unreferenced);
	}
	else if(type == PHONG_SHADING)
	{
		CreateShader("shaders\\phong_shading_vert.vert", "shaders\\phong_shading_frag.frag", Unreferenced);
	}
	else if (type == BLINN_SHADING)
	{
		CreateShader("shaders\\blinnShader_vert.vert", "shaders\\blinnShader_frag.frag", Unreferenced);
	}
}
